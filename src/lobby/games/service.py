...
class GamesService:
    ...
    async def create_game(self, num_bots: int = 3) -> CreateGameResponse:
        await self._registry.check_health()
        healthy_servers = self._registry.get_healthy_servers()
        ...
        await self._create_game_on_server(server, game_id, num_bots)
    ...
    async def _create_game_on_server(self, server: GameServer, game_id: str, num_bots: int = 3) -> None:
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                response = await client.post(
                    f"{server.url}/games",
                    json={"game_id": game_id, "num_bots": num_bots},
                )
