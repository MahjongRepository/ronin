...
class TestHandleDiscard:
    ...
    def test_handle_discard_success(self):
        ...
        result = handle_discard(round_state, game_state, seat=0, data={"tile_id": tile_to_discard})
    ...
    def test_handle_discard_wrong_turn(self):
        ...
        result = handle_discard(round_state, game_state, seat=1, data={"tile_id": 0})
    ...
    def test_handle_discard_missing_tile_id(self):
        game_state = self._create_game_state()
        round_state = game_state.round_state

        result = handle_discard(round_state, game_state, seat=0, data={})

        error_events = [e for e in result.events if isinstance(e, ErrorEvent)]
        assert len(error_events) == 1
        assert error_events[0].code == "missing_tile_id"
...
class TestHandleRiichi:
    ...
    def test_handle_riichi_success(self):
        ...
        result = handle_riichi(round_state, game_state, seat=0, data={"tile_id": tile_to_discard})
    ...
    def test_handle_riichi_wrong_turn(self):
        ...
        result = handle_riichi(round_state, game_state, seat=1, data={"tile_id": 0})
...
class TestHandleRon:
    ...
    def test_handle_ron_success(self):
        ...
        result = handle_ron(
            round_state, game_state, seat=1, data={"tile_id": win_tile, "from_seat": discarder_seat}
        )
    ...
    def test_handle_ron_missing_params(self):
        game_state, _, _ = self._create_ron_opportunity()
        round_state = game_state.round_state

        result = handle_ron(round_state, game_state, seat=1, data={})

        error_events = [e for e in result.events if isinstance(e, ErrorEvent)]
        assert len(error_events) == 1
        assert error_events[0].code == "missing_params"
...
class TestHandlePon:
    ...
    def test_handle_pon_success(self):
        ...
        result = handle_pon(round_state, game_state, seat=1, data={"tile_id": tile_to_pon})
    ...
    def test_handle_pon_missing_tile_id(self):
        game_state, _ = self._create_pon_opportunity()
        round_state = game_state.round_state

        result = handle_pon(round_state, game_state, seat=1, data={})
...
class TestHandleChi:
    ...
    def test_handle_chi_success(self):
        ...
        result = handle_chi(
            round_state, game_state, seat=1, data={"tile_id": tile_to_chi, "sequence_tiles": sequence_tiles}
        )
    ...
    def test_handle_chi_missing_params(self):
        game_state, tile_to_chi, _ = self._create_chi_opportunity()
        round_state = game_state.round_state

        result = handle_chi(round_state, game_state, seat=1, data={"tile_id": tile_to_chi})
...
class TestHandleKan:
    ...
    def test_handle_kan_success(self):
        ...
        result = handle_kan(round_state, game_state, seat=0, data={"tile_id": tile_id, "kan_type": "closed"})

        assert isinstance(result, ActionResult)
        meld_events = [e for e in result.events if isinstance(e, MeldEvent)]
        assert len(meld_events) == 1
        assert meld_events[0].meld_type == "kan"
        assert meld_events[0].kan_type == "closed"

    def test_handle_kan_missing_tile_id(self):
        game_state, _ = self._create_closed_kan_opportunity()
        round_state = game_state.round_state

        result = handle_kan(round_state, game_state, seat=0, data={})
...
class TestHandleKyuushu:
    ...
    def test_handle_kyuushu_success(self):
        ...
        round_end_events = [e for e in result.events if isinstance(e, RoundEndEvent)]
        assert len(round_end_events) == 1
        assert round_end_events[0].result["reason"] == "kyuushu_kyuuhai"
