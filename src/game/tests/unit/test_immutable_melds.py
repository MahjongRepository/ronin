...
class TestCallPonImmutable:
    ...
    def test_call_pon_removes_tiles_from_hand(self):
        """Test that tiles are removed from caller's hand."""
        man_1m = TilesConverter.string_to_136_array(man="1111")
        other_tiles = TilesConverter.string_to_136_array(pin="123456789")
        player0_tiles = (man_1m[0], man_1m[1], *other_tiles)
        players = _create_frozen_players(player0_tiles=player0_tiles)
        round_state = _create_frozen_round_state(players=players, current_player_seat=3)

        new_state, _meld = call_pon(round_state, caller_seat=0, discarder_seat=3, tile_id=man_1m[2])

        # Verify tiles removed (should have 9 tiles remaining from pin)
        assert len(new_state.players[0].tiles) == 9
        # The 1m tiles should be gone
        assert man_1m[0] not in new_state.players[0].tiles
        assert man_1m[1] not in new_state.players[0].tiles

    def test_call_pon_original_state_unchanged(self):
        ...
        original_melds_count = len(round_state.players[0].melds)

        _new_state, _meld = call_pon(round_state, caller_seat=0, discarder_seat=3, tile_id=man_1m[2])

        # Original state unchanged
        assert round_state.players[0].tiles == original_tiles
        assert len(round_state.players[0].melds) == original_melds_count
    ...
    def test_call_pon_clears_ippatsu_for_all_players(self):
        ...
        # All players should have ippatsu cleared
...
class TestImmutableMeldStateIntegrity:
    """Test that immutable meld operations maintain state integrity."""

    def test_original_state_unchanged_across_all_melds(self):
        """Verify original state is never mutated for all meld types."""
        # Setup: Player 0 has four 1m tiles and 2m, 3m for chi
        man_tiles = TilesConverter.string_to_136_array(man="11112233")
        player0_tiles = (*man_tiles[:6], *TilesConverter.string_to_136_array(pin="12345"))
        pon_meld = FrozenMeld(
            meld_type=FrozenMeld.PON,
            tiles=tuple(man_tiles[4:7]),  # 2m pon
            opened=True,
            called_tile=man_tiles[6],
            who=0,
            from_who=3,
        )
        players = _create_frozen_players(player0_tiles=player0_tiles, player0_melds=(pon_meld,))

        # Create states for different tests
        pon_state = _create_frozen_round_state(players=players, current_player_seat=3)
        chi_players = list(_create_frozen_players())
        chi_player_tiles = (man_tiles[4], man_tiles[6], *TilesConverter.string_to_136_array(pin="123456789"))
        chi_players[1] = chi_players[1].model_copy(update={"tiles": chi_player_tiles})
        chi_state = _create_frozen_round_state(players=tuple(chi_players), current_player_seat=0)

        # Store original values
        pon_original_tiles = pon_state.players[0].tiles
        chi_original_tiles = chi_state.players[1].tiles

        # Execute all meld operations
        call_pon(pon_state, caller_seat=0, discarder_seat=3, tile_id=man_tiles[2])
        ...
        call_chi(
            ...
            discarder_seat=0,
            tile_id=man_tiles[0],
            sequence_tiles=(man_tiles[4], man_tiles[6]),
        )

        # Verify originals unchanged
        assert pon_state.players[0].tiles == pon_original_tiles
        assert chi_state.players[1].tiles == chi_original_tiles
