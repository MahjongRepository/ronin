...
class TestCanCallClosedKan:
    ...
    def test_can_call_closed_kan_with_four_tiles(self):
        ...
        result = can_call_closed_kan(player, tile_34=_string_to_34_tile(man="1"), round_state=round_state)

        assert result is True

    def test_cannot_call_closed_kan_with_three_tiles(self):
        # player has only 3 copies
        player = self._create_player(TilesConverter.string_to_136_array(man="111", pin="1"))
        round_state = self._create_round_state(player)
    ...
    def test_cannot_call_closed_kan_wall_too_small(self):
        player = self._create_player(TilesConverter.string_to_136_array(man="1111", pin="1"))
        round_state = self._create_round_state(player, wall_count=1)
    ...
    def test_closed_kan_in_riichi_tile_is_wait(self):
        ...
        # hand: 9m 9m 9m 9m 1m 2m 3m 4m 5m 6m 7m 8m Haku
        # if 9m is a wait, kan should be rejected
        tiles = (
            TilesConverter.string_to_136_array(man="9999")
            + TilesConverter.string_to_136_array(man="12345678")
            + TilesConverter.string_to_136_array(honors="5")
        )
        player = self._create_player(tiles, is_riichi=True)
        round_state = self._create_round_state(player)

        result = can_call_closed_kan(player, tile_34=_string_to_34_tile(man="9"), round_state=round_state)
...
class TestCanCallAddedKan:
    ...
    def test_can_call_added_kan_with_pon_and_fourth_tile(self):
        ...
        player = self._create_player_with_pon(
            [man_1m[3], *TilesConverter.string_to_136_array(pin="1", sou="1", honors="1")],
            pon_tile_34=_string_to_34_tile(man="1"),
        )
        round_state = self._create_round_state(player)

        result = can_call_added_kan(player, tile_34=_string_to_34_tile(man="1"), round_state=round_state)
    ...
    def test_cannot_call_added_kan_no_pon(self):
        ...
        player = MahjongPlayer(
            seat=0,
            name="Test",
            tiles=TilesConverter.string_to_136_array(man="1", pin="1", sou="1", honors="1"),
        )
        round_state = self._create_round_state(player)
    ...
    def test_cannot_call_added_kan_no_fourth_tile(self):
        ...
        player = self._create_player_with_pon(
            TilesConverter.string_to_136_array(pin="1", sou="1", honors="12"),
            pon_tile_34=_string_to_34_tile(man="1"),
        )
        round_state = self._create_round_state(player)
    ...
    def test_cannot_call_added_kan_when_max_kans_reached(self):
        ...
        round_state.players[2].melds = [
            Meld(
                meld_type=Meld.KAN, tiles=TilesConverter.string_to_136_array(pin="4444"), opened=True, who=2
            ),
            Meld(
                meld_type=Meld.SHOUMINKAN,
                tiles=TilesConverter.string_to_136_array(pin="5555"),
                opened=True,
                who=2,
            ),
        ]
